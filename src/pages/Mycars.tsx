...
export default function MyCars() {
  ...
  const handleImageSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fileReader.onload = () => {
      ...
      setNewCar((current) => ({ ...current, image_url: result }));
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div className="mb-4 flex items-center justify-between">
            <div className="rounded-xl bg-primary-50 p-3 text-primary-600 dark:bg-primary-500/10 dark:text-primary-400">
              <CarFront size={22} />
            </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-600 dark:bg-amber-500/10 dark:text-amber-300">
              ตรวจเช็ก
            </span>
          </div>
                  ...
                  <div className="flex flex-1 flex-col justify-between p-6 md:p-8">
                    <div>
                      <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <span
                          className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-bold ${status.badgeClass}`}
                        >
                          <StatusIcon size={16} />
                          {status.label}
                        </span>
                    ...
                    <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-slate-200 pt-6 dark:border-slate-700">
                      ...
                      <button
                        type="button"
                        onClick={() => handlePlaceholderAction('หน้าบันทึกซ่อมของ', car)}
                      ...
                      <div className="ml-auto flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => handlePlaceholderAction('การแก้ไขข้อมูลของ', car)}
          ...
          <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900">
            <div className="flex items-center gap-3 border-b border-slate-200 px-6 py-4 dark:border-slate-800">
              ...
              <div className="flex-1">
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">เพิ่มรถยนต์ใหม่</h2>
              </div>
              <button
                type="button"
                onClick={() => {
            ...
            <form onSubmit={handleAddCar}>
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  ...
                  <label className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      value={newCar.brand}
                      onChange={(event) =>
                        setNewCar((current) => ({ ...current, brand: event.target.value }))
                      }
                      className="h-11 rounded-lg border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-400 focus:border-primary-500 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500"
                  ...
                  <label className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      value={newCar.model}
                      onChange={(event) =>
                        setNewCar((current) => ({ ...current, model: event.target.value }))
                      }
                      className="h-11 rounded-lg border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-400 focus:border-primary-500 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500"
                  ...
                  <label className="flex flex-col gap-1.5">
                    ...
                    <input
                      value={newCar.color}
                      onChange={(event) =>
                        setNewCar((current) => ({ ...current, color: event.target.value }))
                      }
                      className="h-11 rounded-lg border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-400 focus:border-primary-500 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500"
                  ...
                  <label className="flex flex-col gap-1.5">
                    <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
                      เลขไมล์ปัจจุบัน (Mileage)
                    </span>
                    <div className="relative">
                      <input
                        value={newCar.mileage}
                        onChange={(event) =>
                          setNewCar((current) => ({ ...current, mileage: event.target.value }))
                        }
                        className="h-11 w-full rounded-lg border-slate-200 bg-slate-50 pr-12 text-slate-900 placeholder:text-slate-400 focus:border-primary-500 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500"
              ...
              <div className="flex items-center justify-end gap-3 border-t border-slate-200 bg-slate-50/80 px-6 py-4 dark:border-slate-800 dark:bg-slate-800/50">
                <button
                  type="button"
                  onClick={() => {
                    setIsModalOpen(false);
                  ...
                  className="rounded-lg px-6 py-2.5 font-medium text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-300 dark:hover:bg-slate-700"
                >
                  ยกเลิก
                </button>
                ...
                <button
                  type="submit"
                  disabled={isSaving}
                  className="inline-flex items-center gap-2 rounded-lg bg-primary-600 px-6 py-2.5 font-bold text-white shadow-lg shadow-primary-500/20 transition-all hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-70"
                >
                  <Save size={16} />
